 ...
 * \file crr_obj.cc
 * \brief Definition of combined rank and regression objectives.
 * \author Tong Ming
 */
#include <dmlc/omp.h>
...
DMLC_REGISTRY_FILE_TAG(crr_obj);

struct CRRParam : public dmlc::Parameter<CRRParam> {
  int num_pairsample;
  float fix_list_weight;
  float scale_pos_weight;
  float rank_coeff;
  // declare parameters
  DMLC_DECLARE_PARAMETER(CRRParam) {
    DMLC_DECLARE_FIELD(num_pairsample).set_lower_bound(1).set_default(5)
    ...
    DMLC_DECLARE_FIELD(rank_coeff).set_default(0.5f).set_lower_bound(0.0f)
        .describe("Scale the weight of rank gradient by this factor");
  }
};

// objective for lambda rank
class CRRObj : public ObjFunction {
 public:
  ...
  void GetGradient(const std::vector<float> &preds,
    ...
    for (omp_ulong i = 0; i < ndata; ++i) {
      float p = CRRObj::PredTransform(preds[i]);
      float w = info.GetWeight(i) * (1-param_.rank_coeff);
      if (info.labels[i] == 1.0f) w *= param_.scale_pos_weight;
      if (!CRRObj::CheckLabel(info.labels[i])) label_correct = false;
      out_gpair->at(i) = bst_gpair(CRRObj::FirstOrderGradient(p, info.labels[i]) * w,
                                   CRRObj::SecondOrderGradient(p, info.labels[i]) * w);
    }
    if (!label_correct) {
      LOG(FATAL) << CRRObj::LabelErrorMsg();
    }
    ...
    #pragma omp parallel
    {
      ...
      for (bst_omp_uint k = 0; k < ngroup; ++k) {
        ...
        for (unsigned j = gptr[k]; j < gptr[k+1]; ++j) {
          lst.push_back(ListEntry(preds[j], info.labels[j], info.weights[j], j));
        }
        std::sort(lst.begin(), lst.end(), ListEntry::CmpPred);
        rec.resize(lst.size());
        for (unsigned i = 0; i < lst.size(); ++i) {
          rec[i] = std::make_pair(lst[i].label*lst[i].weight, i);
        }
        ...
        for (size_t i = 0; i < pairs.size(); ++i) {
          ...
          const float w = pairs[i].weight * scale * param_.rank_coeff;
 ...
 protected:
  ...
  struct ListEntry {
    ...
    float label;
    /*! \brief wight of the entry */
    float weight;
    /*! \brief row index in the data matrix */
    unsigned rindex;
    // constructor
    ListEntry(float pred, float label, float weight, unsigned rindex)
        : pred(pred), label(label), weight(weight), rindex(rindex) {}
    ...
    inline static bool CmpLabel(const ListEntry &a, const ListEntry &b) {
      return a.label*a.weight > b.label*b.weight;
    }
  };
...
// beta version: CRR with NDCG lambda rank
class CRRObjNDCG : public CRRObj {
 protected:
...
DMLC_REGISTER_PARAMETER(CRRParam);


XGBOOST_REGISTER_OBJECTIVE(CRRRankNDCG, "crr:ndcg")
.describe("combine rank and logistic with NDCG as rank objective.")
.set_body([]() { return new CRRObjNDCG(); });

XGBOOST_REGISTER_OBJECTIVE(CRRRankObjMAP, "crr:map")
.describe("combine rank and logistic with MAP as rank objective.")
.set_body([]() { return new CRRObjMAP(); });
